...
class TestGetAccountArg:
    ...
    @patch("snakemake_executor_plugin_slurm.validate_account")
    def test_string_account(self, mock_validate_account, mock_get_account, tmp_path):
        ...
        mock_validate_account.assert_called_with("123456789", executor.logger)

    @patch("snakemake_executor_plugin_slurm.get_account")
    @patch("snakemake_executor_plugin_slurm.validate_account")
    def test_integer_account(self, mock_validate_account, mock_get_account, tmp_path):
    ...
    def test_multiple_accounts_string(
        self, mock_validate_account, mock_get_account, tmp_path
    ):
    ...
    @patch("snakemake_executor_plugin_slurm.get_account")
    @patch("snakemake_executor_plugin_slurm.validate_account")
    def test_guessed_account_uses_canonical_value(
        self, mock_validate_account, mock_get_account, tmp_path
    ):
        """Guessed accounts use the canonical validated value in the sbatch arg."""
        executor = self._make_executor_stub()
        job = _make_mock_job()

        mock_get_account.return_value = "nhr_zdvhpc"
        mock_validate_account.return_value = "nhr-zdvhpc"
