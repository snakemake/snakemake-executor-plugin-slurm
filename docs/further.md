...
To get a template for a partition configuration file, run `generate-slurm-partition-config`.
By default it prints the YAML to stdout; use `-o partitions.yaml` to write directly to a file.
The output should be reviewed and edited, as not all information can be deduced from SLURM's
partition configuration (e.g. which partitions support MPI jobs).
The resulting file can be used with `--slurm-partition-config` or saved alongside a profile;
set the environment variable `$SNAKEMAKE_SLURM_PARTITIONS` to point to it for automatic pickup.
Run `generate-slurm-partition-config -h` to see all options.
